...
# 특정 버전으로 빌드
docker build -t label-studio-custom:1.20.0-sso.1 .
...
docker tag label-studio-custom:local ghcr.io/heartyoh/label-studio-custom:1.20.0-sso.1

# 3. latest 태그도 추가
docker tag label-studio-custom:local ghcr.io/heartyoh/label-studio-custom:latest

# 4. 이미지 푸시
docker push ghcr.io/heartyoh/label-studio-custom:1.20.0-sso.1
docker push ghcr.io/heartyoh/label-studio-custom:latest

# 5. 푸시 확인
# https://github.com/heartyoh?tab=packages
```
...
1. **GitHub 저장소 생성**:
   ...
   # GitHub에서 저장소 생성 후
   git remote add origin https://github.com/heartyoh/label-studio-custom.git
   git branch -M main
...
# 1. 코드 변경사항 커밋
git add .
git commit -m "feat: Release v1.20.0-sso.1"

# 2. 태그 생성
git tag v1.20.0-sso.1

# 3. 태그 푸시
git push origin v1.20.0-sso.1
...
3. GitHub Container Registry에 푸시:
   - `ghcr.io/heartyoh/label-studio-custom:1.20.0-sso.1`
   - `ghcr.io/heartyoh/label-studio-custom:latest`
4. GitHub Release 생성
...
- v1.20.0-sso.2  → Label Studio 1.20.0 기반, 두 번째 버전 (버그 수정)
...
```bash
# Public 이미지 pull
docker pull ghcr.io/heartyoh/label-studio-custom:1.20.0-sso.1

# 또는 latest
docker pull ghcr.io/heartyoh/label-studio-custom:latest
```
...
services:
  labelstudio:
    # 특정 버전 사용 (권장)
    image: ghcr.io/heartyoh/label-studio-custom:1.20.0-sso.1

    # 또는 latest (개발용)
    # image: ghcr.io/heartyoh/label-studio-custom:latest
...
docker buildx build \
  ...
  -t ghcr.io/heartyoh/label-studio-custom:1.20.0-sso.1 \
  --push \
  .
```
...
# 태그 및 푸시
docker tag label-studio-custom:local heartyoh/label-studio-custom:1.20.0-sso.1
docker push heartyoh/label-studio-custom:1.20.0-sso.1
```
...
docker tag label-studio-custom:local \
  <account-id>.dkr.ecr.ap-northeast-2.amazonaws.com/label-studio-custom:1.20.0-sso.1
docker push <account-id>.dkr.ecr.ap-northeast-2.amazonaws.com/label-studio-custom:1.20.0-sso.1
```
...
# 토큰 재생성 후 재로그인
echo $NEW_GITHUB_TOKEN | docker login ghcr.io -u heartyoh --password-stdin
```
